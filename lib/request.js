...
function buildRequest (opts) {
  ...
  if (isHttp2) {
    ...
    if (opts.undici.tls !== 'object') {
      opts.undici.tls = {}
    }
    opts.undici.tls.rejectUnauthorized = opts.undici.tls.rejectUnauthorized || false
    undiciAgent = new undici.Agent(opts.undici)
  ...
  function handleUndici (opts, done) {
    ...
    if (opts.url.protocol.indexOf('unix') === 0) {
      done(new Error('unix socket not supported with undici yet'))
      return
    }
    const pool = undiciAgent.get(baseUrl || opts.url.origin)
