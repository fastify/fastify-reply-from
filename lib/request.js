...
function buildRequest (opts) {
  ...
  function handleHttp1Req (opts, done) {
    if (opts.certificate && opts.url.protocol) {
      if (Buffer.isBuffer(opts.certificate.key) && Buffer.isBuffer(opts.certificate.cert)) {
        agents[opts.url.protocol].cert = opts.certificate.cert
        agents[opts.url.protocol].key = opts.certificate.key
      } else {
        throw new Error('key and cert must be a buffer')
      }
    }
