...
function buildRequest (opts) {
  ...
  if (isHttp2) {
    ...
    http2Client = http2.connect(opts.base, { rejectUnauthorized: opts.rejectUnauthorized })
  } else {
    agents = {
  ...
  function request (opts, done) {
    if (isHttp2) {
      handleHttp2Req(opts, done)
    } else {
      handleHttp1Req(opts, done)
    }
  }
