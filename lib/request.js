...
function buildRequest (opts) {
  ...
  if (isHttp2) {
    ...
    // if (opts.certificate) {
    //   if (Buffer.isBuffer(opts.certificate.key) && Buffer.isBuffer(opts.certificate.cert)) {
    //     agents['https'].cert = opts.certificate.cert
    //     agents['https'].key = opts.certificate.key
    //   }
    // }
  }
  ...
  function handleHttp1Req (opts, done) {
    if (opts.certificate) {
      agents['clone'] = new https.Agent(agentOption({
        ...opts, ...opts.certificate
      }))
      // if (Buffer.isBuffer(opts.certificate.key) && Buffer.isBuffer(opts.certificate.cert)) {
      //
      // }
    ...
    const req = requests[opts.url.protocol].request({
      ...
      agent: opts.certificate ? agents['clone'] : agents[opts.url.protocol]
    })
    req.on('error', done)
    req.on('response', res => {
...
function agentOption (opts) {
  return {
    ...
    cert: opts.certificate.cert || null,
    key: opts.certificate.key || null
  }
}
